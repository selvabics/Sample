...
open class CircularTransition: NSObject, UIViewControllerAnimatedTransitioning, UIViewControllerTransitioningDelegate {
    ...
    convenience public init(animatedView: UIView, colorCode: Int) {
        self.init()
        self.animatedView = animatedView
        self.startBackgroundColor = UIColor(netHex: colorCode)
    }
    ...
    open func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if !self.isReverse {
            ...
            UIView.animate(withDuration: self.transitionDuration(using: transitionContext) * 0.42, delay: self.transitionDuration(using: transitionContext) * 0.58, animations: {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + self.transitionDuration(using: transitionContext) * 0.32) {
                UIView.transition(with: animatedViewForTransition, duration: self.transitionDuration(using: transitionContext) * 0.58, options: [], animations: {
